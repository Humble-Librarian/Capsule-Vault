...
// Direct IndexedDB read as fallback
...
async function loadStats() {
  ...
  if (r?.ok) {
    document.getElementById("stats-label").textContent = `${r.count} / ${r.max} memories`;
  }
}
...
function renderList() {
  ...
  if (!filtered.length) {
    ...
    d.textContent = "No memories match your search.";
    c.appendChild(d);
    return;
  }
...
function bindEvents() {
  ...
  document.getElementById("btn-export")?.addEventListener("click", async () => {
    ...
    toast(`Exported ${r.data.memories.length}`);
  });
  ...
  document.getElementById("import-file")?.addEventListener("change", async e => {
    ...
    try {
      ...
      await loadMemories(); renderList(); await loadStats();
      toast(`Imported ${r.imported}`);
